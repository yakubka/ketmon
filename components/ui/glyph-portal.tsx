...
export default function GlyphPortal({
  word = "SWITCH", focusChar, interactive = true, background, front, children, scrollLength = 2.4,
  ...
  const text = word.trim().normalize("NFC") || "SWITCH";
  ...
  return (
    <section ref={sectionRef} id={uid} className={className} aria-label={text}
      ...
      <div data-gp-pin>
        <div data-gp-field aria-hidden="true" {...{inert: ""} as any}>
          {background ?? <div style={{ position: "absolute", inset: 0, transform: "scale(var(--gp-field-scale,1))", background: "radial-gradient(circle at 18% 8%, rgba(20,184,166,.72), transparent 34%), radial-gradient(circle at 82% 20%, rgba(251,251,250,.12), transparent 28%), radial-gradient(circle at 48% 78%, rgba(13,148,136,.5), transparent 44%), linear-gradient(135deg,#0d9488 0%,#14b8a6 48%,#0f766e 100%)" }} />}
        </div>
        ...
        <div data-gp-choices role="radiogroup" aria-label="Choose the letter to enter through" {...{inert: ""} as any}>
        ...
        <div data-gp-caption>
          <span data-gp-hint aria-hidden="true">{interactive ? "Scroll to enter." : ""}</span>
